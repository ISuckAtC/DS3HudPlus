...
namespace Macros
{
    ...
    class Program
    {
        ...
        static void Wait(int milliseconds, string message = "")
        {
            if (message != "")
            {
                Console.WriteLine("Waiting " + milliseconds + "ms (" + message + ")");
            }
        ...
        static public void MacroLoop()
        {
            RefreshKeys();
            bool exit = false;
            while(!exit)
            {
                if (GetKeyDown(WinVirtualKey.VK_ESCAPE)) exit = true;
                if (GetKeyDown(WinVirtualKey.VK_MBUTTON))
                {
                    SendKey(ScanCodes.Space);
                    Wait(FramesToMs(1));
                    SendKey(ScanCodes.Space, true);
                }
                RefreshKeys();
            }
        }
        ...
        public static void CheckStuff()
        {
            ...
            do 
            {
                if (GetKeyDown(WinVirtualKey.VK_W))
                {
                    w = true;
                    Console.WriteLine("W key pressed in " + sw.ElapsedMilliseconds + "ms");
                }
                if (GetKeyDown(WinVirtualKey.VK_XBUTTON2))
                {
                    x = true;
                    Console.WriteLine("XBUTTON2 key pressed in " + sw.ElapsedMilliseconds + "ms");
                }
                if (GetKeyDown(WinVirtualKey.VK_LBUTTON))
                {
                    l = true;
                    Console.WriteLine("LBUTTON key pressed in " + sw.ElapsedMilliseconds + "ms");
                }
            } while (!(w && x && l));
        ...
        {
            return (int)((double)frames * (1000.0 / 60.0));
        }

        public static void MainHandWeirdStance()
        {
            SendKey(ScanCodes.P);
            Wait(FramesToMs(2));
            SendKey(ScanCodes.LeftShift);

            Wait(FramesToMs(5));

            SendKey(ScanCodes.P, true);
            SendKey(ScanCodes.LeftShift, true);
            ...
            SendKey(ScanCodes.C, true);

            Wait(FramesToMs(15));

            SendKey(ScanCodes.W);
            Console.WriteLine("Sent W");
            ...
            Wait(50);
            SendKey(ScanCodes.LeftShift);
            Console.WriteLine("Sent LeftShift");

            Wait(FramesToMs(10));

            SendKey(ScanCodes.P, true);
            SendKey(ScanCodes.LeftShift, true);
            SendKey(ScanCodes.W, true);
        }
        ...
        public static void MainhandWalmart()
        {
            SendKey(ScanCodes.Space);
            SendKey(ScanCodes.W);
            Wait((int)(16.666 * 120));
            ...
            SendKey(ScanCodes.Escape);
            Wait((int)(16.666 * 2));
            SendKey(ScanCodes.Escape, true);
            SendKey(ScanCodes.E);
            Wait((int)(16.666 * 2));
            SendKey(ScanCodes.E, true);
